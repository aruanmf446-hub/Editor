...
export function updatePlayerMovement(world: RuntimeWorld, delta: number) {
  ...
  if (player.grounded) player.coyoteRemaining = RUNTIME_CONFIG.coyoteTime;
  ...
  if (!player.crouching && player.jumpBufferRemaining > 0 && (player.grounded || player.coyoteRemaining > 0)) {
