...
export function ScenePanel() {
  ...
  return (
    <aside className="panel scenes-panel">
      <div className="panel-title-row"><h2>Cenas</h2><button className="icon-button" onClick={addScene} title="Nova cena">＋</button></div>
      <div className="scene-list">
        {project.scenes.map((scene, index) => (
          <div className={`scene-row ${scene.id === selectedSceneId ? 'active' : ''}`} key={scene.id}>
            <button className="scene-select" onClick={() => selectScene(scene.id)}><span>{index + 1}</span>{scene.name}</button>
